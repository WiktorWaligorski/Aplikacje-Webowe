    ...
    if (pathname === "/") {
      ...
      <body><h1>HTML wygenerowanym za pomocą Node.js</h1></body>
        ...
        try {
            ...
            console.log("Otrzymane parametry GET:", paramsArray);

            const timestamp = Date.now();
            const filename = `params_${timestamp}.json`;
            const filePath = path.join(__dirname, filename);

            await fs.writeFile(filePath, JSON.stringify(paramsArray, null, 2), "utf-8");

            res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
            res.end(JSON.stringify({ ok: "ok" }));
        } catch (err) {
            console.error("Błąd zapisu parametrów:", err);
